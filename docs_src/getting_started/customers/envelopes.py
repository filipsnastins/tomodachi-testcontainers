"""Tomodachi SNS/SQS JSON message envelope used by the customers example tests.

This is a local copy of ``tomodachi.envelope.json_base.JsonBase`` so that the
documentation example tests do not depend on ``tomodachi`` directly. ``tomodachi``
constrains ``aiobotocore<3``, which is incompatible with ``tomodachi-testcontainers``
(``aiobotocore>=3``), so the two packages can no longer be installed together.
"""
